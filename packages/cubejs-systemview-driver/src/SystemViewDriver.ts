...
// TODO: convert to SystemViewDriverConfiguration
export interface SystemViewDriverConfiguration {
  readOnly?: boolean;
 ...
 * MySQL driver class.
 */
export class SystemViewDriver extends BaseDriver implements DriverInterface {
  /**
  ...
  protected readonly config: SystemViewDriverConfiguration;
   ...
   */
  public constructor(
    config: SystemViewDriverConfiguration & {
      /**
       * Data source name.
       */
      dataSource?: string;
  ...
  // TODO: implement using connection config.
  ...
  // TODO: only required to implement this function and testConnection. All others can fallback to base-driver.
  ...
  public informationSchemaQuery() {
    return `${super.informationSchemaQuery()} AND columns.table_schema = '${
      ...
      for (let j = 0; j < Math.ceil(tableData.rows.length / batchSize); j++) {
        ...
        const valueParamPlaceholders = indexArray
          .map(
            (i) =>
              `(${columns
                .map((c, paramIndex) =>
                  this.param(paramIndex + i * columns.length)
                )
                .join(", ")})`
          )
          .join(", ");
        const params = indexArray
          .map((i) =>
            columns.map((c) =>
              this.toColumnValue(
                tableData.rows[i + j * batchSize][c.name],
                c.type
              )
            )
          )
  ...
  public toGenericType(columnType: string) {
    return (
      MySqlToGenericType[columnType.toLowerCase()] ||
